...
This module contains the integration and live test suites for the `code-wiki-gen` package. These tests validate end-to-end behavior, determinism, failure handling, publishing workflows, and SDK contract compliance — ranging from isolated integration tests with mocks to live tests that run against real infrastructure.
...
| File | Description |
|------|-------------|
| `code-wiki-gen/test/integration/determinism.test.ts` | Asserts that repeated runs with the same input produce identical wiki output |
| `code-wiki-gen/test/integration/e2e.test.ts` | Full end-to-end pipeline test from repository ingestion to wiki page output |
| `code-wiki-gen/test/integration/failure.test.ts` | Exercises error paths — malformed input, network failures, partial results |
| `code-wiki-gen/test/integration/publish.test.ts` | Tests the publish step including formatting, file output, and destination routing |
| `code-wiki-gen/test/integration/sdk-contract.test.ts` | Validates the public SDK surface against expected input/output contracts |

### Live Tests

| File | Description |
|------|-------------|
| `code-wiki-gen/test/live/generate.live.test.ts` | Runs wiki generation against a real LLM provider to validate live output quality |
| `code-wiki-gen/test/live/publish.live.test.ts` | Publishes output to a real target to verify end-to-end delivery |
