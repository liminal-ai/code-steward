...
The Configuration module handles loading, merging, and resolving configuration for the code-wiki-gen tool. It implements a layered config strategy: built-in defaults → file-based config → CLI arguments.
...
| File | Purpose |
|---|---|
| `code-wiki-gen/src/config/defaults.ts` | `getDefaults` — returns the baseline default configuration object |
| `code-wiki-gen/src/config/file-loader.ts` | `loadConfigFile` — reads and parses the config file (`CONFIG_FILE_NAME`) from disk |
| `code-wiki-gen/src/config/resolver.ts` | `resolveConfiguration` — merges defaults + file config + overrides into a validated config |
| `code-wiki-gen/src/cli/config-merger.ts` | `mergeCliConfiguration`, `mergeAnalyzeRequest`, `mergeRunRequest`, `mergeStatusRequest`, `mergePublishRequest` — per-command CLI arg merging; also exports `ConfigurableCliArgs` and `PublishCliArgs` interfaces |
