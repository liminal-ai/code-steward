...
Comprehensive test suites for the `code-wiki-gen` tool, covering all major subsystems. These tests validate correctness of analysis, CLI behavior, configuration, environment checks, metadata tracking, orchestration workflows, prompt construction, and input validation.

## Structure

Tests are organized under `code-wiki-gen/test/` mirroring the source layout in `code-wiki-gen/src/`:
...
Tests use the standard TypeScript test runner configured for the project. Run from the `code-wiki-gen` directory.
