...
| File | Export | Purpose |
|------|--------|---------|
| `code-wiki-gen/src/prompts/clustering.ts` | `buildClusteringPrompt` | Builds the prompt for grouping repository components into logical modules |
| `code-wiki-gen/src/prompts/module-doc.ts` | `buildModuleDocPrompt` | Builds the prompt for generating a single module's wiki documentation page |
| `code-wiki-gen/src/prompts/overview.ts` | `buildOverviewPrompt` | Builds the prompt for generating the top-level wiki overview page |
| `code-wiki-gen/src/prompts/quality-review.ts` | `buildQualityReviewPrompt` | Builds the prompt for LLM-based quality review of generated wiki content |
