...
describe("generateDocumentation failure handling", () => {
  ...
  it("TC-5.2a: module generation failure", async () => {
    ...
    const result = expectFailure(
      await generateDocumentation({ mode: "full", repoPath }, (event) => {
        events.push(event);
      }),
    );
  ...
  it("Agent SDK network timeout is surfaced with the failing stage", async () => {
    ...
    const planningFailure = expectFailure(
      await generateDocumentation({ mode: "full", repoPath: planningRepoPath }),
    );
    ...
    const overviewFailure = expectFailure(
      await generateDocumentation({ mode: "full", repoPath: overviewRepoPath }),
    );
