...
const buildAnalysis = (
  ...
  return {
    ...
    repoPath: overrides.repoPath ?? "/tmp/documentation-engine",
    summary: {
...
const createRepo = (config?: Record<string, unknown>): string => {
  ...
  if (config) {
    writeFileSync(
      path.join(repoPath, ".docengine.json"),
...
const runHappyPath = async (repoPath = createRepo()) => {
  ...
  return expectSuccess(await generateDocumentation({ mode: "full", repoPath }));
};
...
describe("generateDocumentation", () => {
  it("TC-1.1a: successful full generation", async () => {
    ...
    const result = expectSuccess(
      await generateDocumentation({ mode: "full", repoPath }),
    );
  ...
  it("TC-1.2b: defaults fill unset fields", async () => {
    ...
    expectSuccess(await generateDocumentation({ mode: "full", repoPath }));
