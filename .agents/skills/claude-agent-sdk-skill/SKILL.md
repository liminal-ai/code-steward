...
```bash
npm install @anthropic-ai/Codex-agent-sdk
...
Returns an async iterator yielding messages as the agent works through its loop (prompt → Codex responds → execute tools → repeat):
...
for await (const message of query({
  ...
  switch (message.type) {
    ...
    case "assistant": // Codex's responses — text + tool_use blocks in message.content
...
```typescript
import { query, tool, createSdkMcpServer } from "@anthropic-ai/Codex-agent-sdk";
import { z } from "zod";
